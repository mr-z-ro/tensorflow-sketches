...
Also demonstrates use of TensorFlow data readers.

We will use some popular filters (kernels) for our image.
...
a tf.train.QueueRunner is added to the graph, which also must be run using e.g. tf.train.start_queue_runners() else your session will run into a deadlock and your program will crash.

And to run QueueRunner, you need a coordinator to close your queue for you.
...
Borrowed from Chip Huyen at sc20si.stanford.edu
"""
...
FILENAME = 'data/man.jpg'

def read_one_image(filename):
    """ This is just to demo how to open an image in TF, but it's actually a lot easier to use Pillow"""
    filename_queue = tf.train.string_input_producer([filename])
    image_reader = tf.WholeFileReader()
    _, image_file = image_reader.read(filename_queue)
    image = tf.image.decode_jpeg(image_file, channels=3)
    image = tf.cast(image, tf.float32) / 256.0 # need float for conv2d
    return image
